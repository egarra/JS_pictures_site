...
var accordion = function accordion(triggerSelector, blockContainer) {
...
var forms = function forms(state) {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      var dots;
      var arr = item.files[0].name.split('.'); //получаем первый файл, который загрузил пользователь, затем имя этого файла, и делим его на часть до точки и после нее. получаем массив из двух файлов
      ...
      var name = arr[0].substring(0, 6) + dots + arr[1]; //обрезаем первый файл из массива до количества 5 символов, добавляем dots и второй файл из массива(то, что идет после точек)
      ...
      if (item.closest('.first_upload') && item.files) {
        var divBtn = document.querySelector('.first_upload'),
        ...
        item.addEventListener('click', function (e) {
          if (item.closest('.first_upload') && item.files && divBtn.childNodes[3].textContent !== "Файл не выбран") {
            e.preventDefault();
            var formData = new FormData(divBtn.parentElement);
            Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(path.designer, formData).then(function (res) {
              console.log(res);
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        console.log(document.querySelector('.fixed-gift'));
      });
    });
